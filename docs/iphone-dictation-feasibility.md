# iPhone Dictation App — Feasibility & Build Plan

## Context

The goal: hold a button, speak, and have cleaned-up text — filler words removed,
punctuation added — land in whatever app you are currently typing in. The same
experience desktop dictation tools give you, on iPhone.
...
- **Audience:** personal use first, but no architectural choices that would block a later
  App Store release.
- **Transcription:** cloud ASR + LLM cleanup.
...
**Evidence for.** At least one dictation keyboard ships exactly this on the App Store, with
a large and largely positive review count — so whatever the documentation says, the
capability exists. The setup instructions such apps publish are consistent with each other
and state the requirements plainly:

- Full Access must be enabled: *Settings → General → Keyboard → Keyboards → [keyboard] →
  Allow Full Access.*
- **Microphone permission must be granted from the main app — it cannot be granted from the
  keyboard.**
- Missing Full Access presents as a microphone failure, so it is the first thing to check
  when dictation does not start.

That last point is telling: it describes precisely the symptom the failing forum posts
report.
...
the good implementations feel instant. Encode to Opus or AAC to cut upload size. Target p50 under ~1.2s from
...
recurring cost floor is why the commercial products in this category charge a monthly
subscription rather than selling the app outright — worth knowing if this ever stops being
personal.
...
> The claim that a shipping keyboard requires Full Access plus container-app microphone
> permission comes from the published setup documentation of commercial dictation
> keyboards on the App Store. Those vendor links are deliberately not cited here, so
> verify that requirement yourself during the Phase 0 spike rather than taking it on
> trust.
